...
;
; PURPOSE:
;  To construct full sun 3D data cube from the slit scan of NE VIII [June 07, 1996]
...
fps=1
ovid=idlffvideowrite('fullsun_NEVIII_06071996.mp4')
...
for i=0 ,count-1 do begin ;doing so will give i=0,i=2,i=5
	...
	if dimention(1) eq 1 and dimention(2) eq 360 then begin ;eq is 1 since NAXIS is 1, NAXIS2 is 360
      		...
      		xss=ROUND((CRVAL3+1024))/3 ;1500 gives us the indices for the array (value)
      		...
      		if abs(ys0-yss) ge 250 then begin
      			...
      			strip_no=strip_no+1
      			window,xs=800,ys=800
      			plot_image,sqrt(max(rastor,dimension=1)),title='Full Disk Peak Intensity in Ne VIII (June 7, 1996)', xtitle='arcseconds',ytitle='arcseconds'
